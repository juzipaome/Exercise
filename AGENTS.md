...
- MIUIX 0.9.4、Room、DataStore、Coroutines/Flow、Coil GIF
...
- 页面进出和返回必须直接使用 MIUIX 0.9.4 `miuix-nav` 的 `NavDisplay`，不得只复制其曲线自行实现；主 Tab Pager 遵循 0.9.4 示例的 `MainPagerState` 行为。
...
数据库版本为 6，Room schema 保存在 `app/schemas/`；本地 Robolectric 测试覆盖 5→6 升级、训练事务、备份和休息提醒。Compose 仪器化回归覆盖保存失败后的输入保留与重试、日历切月/日期详情、动作开始后的排序、手动拖动边缘滚动、防误删与卡片位置稳定性；设置页持久化 MIUIX 0.9.4 的增强分页手势、渐进模糊和大屏对话框开关。测试依赖只用于 Debug/测试，版本与 MIUIX 实际解析的 Compose 对齐。
